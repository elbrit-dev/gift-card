...
const InProgressCardsTable: React.FC<InProgressCardsTableProps> = ({
  ...
  const pagedCards = formattedCards.slice(
    (page - 1) * pageSize,
    page * pageSize
  );
  ...
  const downloadSelectedAsCSV = () => {
    ...
    const csv = [
      ["Gift Card No", "Sales Team", "HQ", "Status", "Doctor Name", "Doctor Phone", "Employee Name", "Employee Designation", "Employee Phone", "Expiry"],
      ...selectedData.map((c) => [
        c.cardNo, c.salesTeam, c.hq, c.status,
        c.drName, c.drPhoneNumber, c.empName, c.designation,
        c.empPhone, c.expiry
      ]),
    ]
      .map((row) => row.join(","))
     ...
     <div className="overflow-x-auto">
      <table className="text-sm min-w-[1200px] w-full border-collapse border border-gray-200 dark:border-gray-700">
        ...
        <thead>
          <tr className="bg-gray-50 dark:bg-gray-800">
            ...
            {[
              ...
              <th
                key={idx}
                className={`px-3 py-2 font-medium border border-gray-200 dark:border-gray-700 ${
                  ["Doctor Name", "Employee Name", "Employee Designation"].includes(title)
                    ? "text-left text-gray-700 dark:text-white"
                    : "text-center text-gray-700 dark:text-white"
                }`}
              >
                {title}
              </th>
        ...
        <tbody>
          {pagedCards.length === 0 ? (
            <tr>
              <td colSpan={11} className="text-center py-6 text-gray-400 dark:text-gray-500 border border-gray-200 dark:border-gray-700">
