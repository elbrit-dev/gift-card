...
interface DoctorScanPendingCard {
  cardNo: string;
  kit: string;
  SL: string;
  salesTeam: string;
...
interface DoctorScanPendingCardsTableProps {
  cards?: DoctorScanPendingCard[];
  pageSize?: number;
  title?: string;
}

const DoctorScanPendingCardsTable: React.FC<DoctorScanPendingCardsTableProps> = ({
  cards = [],
  pageSize = 5,
  title = "Doctor Scan Pending",
}) => {
  ...
  const downloadSelectedAsCSV = () => {
    ...
    a.href = url;
    a.download = "doctor_scan_pending_cards.csv";
    a.click();
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 mb-6 overflow-x-auto shadow-sm">
      ...
      <table className="min-w-[1300px] w-full text-sm border-collapse border border-gray-200 dark:border-gray-700">
        <thead>
          <tr className="bg-gray-50 dark:bg-gray-800">
            ...
            {["Gift Card No", "Kit", "SL", "Sales Team", "HQ", "Status",
              "Doctor Phone", "Doctor Code", "Doctor Name", "Verify Name", "Verify Score",
              "Emp Name", "Designation", "Emp Phone", "Expiry"]
              .map((title, idx) => (
                <th
                  key={idx}
        ...
        <tbody>
          {pagedCards.length === 0 ? (
            <tr>
              <td colSpan={15} className="text-center py-6 text-gray-400 dark:text-gray-500 border border-gray-200 dark:border-gray-700">
                No cards marked as 'formfilled'
              </td>
            </tr>
          ) : (
            pagedCards.map((card, idx) => (
              <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition text-gray-800 dark:text-white">
                ...
                <td className="px-3 py-2 font-mono break-all border dark:border-gray-700">{card.cardNo}</td>
                <td className="px-3 py-2 font-mono break-all border dark:border-gray-700">{card.kit}</td>
                <td className="px-3 py-2 font-mono break-all border dark:border-gray-700">{card.SL}</td>
                ...
                <td className="px-3 py-2">
                  <span className="inline-block px-2 py-1 rounded-full bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200 text-xs font-semibold">
                    {card.status}
                  </span>
                </td>
                ...
                <td className="px-3 py-2 border dark:border-gray-700">{card.verifyName}</td>
...
export default DoctorScanPendingCardsTable;